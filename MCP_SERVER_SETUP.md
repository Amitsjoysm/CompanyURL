...
```json
{
  ...
  "server": {
    "url": "https://crm-sync-hub-2.preview.emergentagent.com",
    "api_base": "/api"
  },
  "authentication": {
    "type": "api_key",
...
```bash
curl -H "X-API-Key: your_token" \
  https://crm-sync-hub-2.preview.emergentagent.com/api/crawl/search?query=technology
```
...
```bash
curl -H "X-API-Key: your_token" \
  https://crm-sync-hub-2.preview.emergentagent.com/api/crawl/request/req_123
```
...
curl -X POST \
  ...
  https://crm-sync-hub-2.preview.emergentagent.com/api/webhooks
```
