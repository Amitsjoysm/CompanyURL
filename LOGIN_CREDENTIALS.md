...
#### Using Frontend:
1. Go to `https://service-restart-auth.preview.emergentagent.com`
...
# Create API token
curl -X POST https://service-restart-auth.preview.emergentagent.com/api/api-tokens/ \
...
curl -X GET https://service-restart-auth.preview.emergentagent.com/api/users/me \
...
curl -X GET https://service-restart-auth.preview.emergentagent.com/api/api-tokens/ \
  -H "X-API-Key: $API_TOKEN"

# Toggle token (disable/enable)
curl -X PUT https://service-restart-auth.preview.emergentagent.com/api/api-tokens/TOKEN_ID/toggle \
  -H "X-API-Key: $API_TOKEN"

# Delete token
curl -X DELETE https://service-restart-auth.preview.emergentagent.com/api/api-tokens/TOKEN_ID \
  -H "X-API-Key: $API_TOKEN"
```
...
```bash
curl -X GET https://service-restart-auth.preview.emergentagent.com/api/hubspot/status \
  -H "Authorization: Bearer $JWT_TOKEN"
...
```bash
curl -X GET https://service-restart-auth.preview.emergentagent.com/api/hubspot/auth/url \
  -H "Authorization: Bearer $JWT_TOKEN"
...
```bash
curl -X POST https://service-restart-auth.preview.emergentagent.com/api/crawl/single \
  -H "X-API-Key: $API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "input_type": "domain",
...
#### View Crawl History:
```bash
curl -X GET https://service-restart-auth.preview.emergentagent.com/api/crawl/history \
  -H "X-API-Key: $API_TOKEN"
```

#### Search Central Ledger:
```bash
curl -X GET https://service-restart-auth.preview.emergentagent.com/api/crawl/search?query=openai \
  -H "X-API-Key: $API_TOKEN"
```
...
#### Get Available Plans:
```bash
curl -X GET https://service-restart-auth.preview.emergentagent.com/api/payment/plans \
  -H "X-API-Key: $API_TOKEN"
```

#### Create Payment Order:
```bash
curl -X POST https://service-restart-auth.preview.emergentagent.com/api/payment/create-order \
  -H "Authorization: Bearer $JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
