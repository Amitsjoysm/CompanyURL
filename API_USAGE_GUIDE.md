...
curl -H "Authorization: Bearer eyJ0eXAiOiJKV1QiLCJhbGc..." \
  https://api-token-repair.preview.emergentagent.com/api/crawl/history
```
...
curl -X POST \
  ...
  https://api-token-repair.preview.emergentagent.com/api/crawl/single
```
...
curl -X POST \
  ...
  -F "input_type=domain" \
  https://api-token-repair.preview.emergentagent.com/api/crawl/bulk-upload
```
...
curl -H "X-API-Key: your_api_key" \
  https://api-token-repair.preview.emergentagent.com/api/crawl/request/req_xyz789
```
...
```bash
curl -H "X-API-Key: your_api_key" \
  https://api-token-repair.preview.emergentagent.com/api/crawl/history?limit=10
```
...
```bash
curl -H "X-API-Key: your_api_key" \
  "https://api-token-repair.preview.emergentagent.com/api/crawl/search?query=fintech&limit=20"
```
...
curl -H "Authorization: Bearer jwt_token" \
  https://api-token-repair.preview.emergentagent.com/api/api-tokens
```
...
curl -X DELETE \
  -H "Authorization: Bearer jwt_token" \
  https://api-token-repair.preview.emergentagent.com/api/api-tokens/token_123
```
...
curl -X PUT \
  -H "Authorization: Bearer jwt_token" \
  https://api-token-repair.preview.emergentagent.com/api/api-tokens/token_123/toggle
```
...
API_URL = "https://api-token-repair.preview.emergentagent.com/api"
...
const API_URL = 'https://api-token-repair.preview.emergentagent.com/api';
...
```bash
curl -H "X-API-Key: corp_your_key" \
  "https://api-token-repair.preview.emergentagent.com/api/crawl/search?query=saas&limit=20"
```
